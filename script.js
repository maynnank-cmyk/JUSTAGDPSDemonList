...
function toggleLanguage() {
    ...
    // Триггерим событие для обновления auth UI
...
async function initializeData() {
    ...
    // Добавляем обработчик для кнопки входа в мобильном меню
...
// ========== ДОБАВЛЯЕМ ПОДДЕРЖКУ ДЛЯ КНОПОК В ДЕМОНАХ ==========
// Функция для отметки демона как пройденного
window.markDemonAsCompleted = async function(demonId, demonName, points) {
    ...
    const result = window.auth.addCompletedDemon(demonId, demonName, points);
    
    if (result.success) {
        // Показываем уведомление
        const notification = document.createElement('div');
        notification.className = 'auth-message success';
