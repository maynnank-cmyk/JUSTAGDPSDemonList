...
// ========== ВЫБОР ТИПА ПОИСКА ==========
let currentSearchType = localStorage.getItem('challengeSearchType') || 'normal';
...
function changeSearchType(type) {
    ...
    localStorage.setItem('challengeSearchType', type);
...
function loadSearchType() {
    const savedType = localStorage.getItem('challengeSearchType') || 'normal';
...
function updateIconColors(theme) {
    ...
    searchIcons.forEach(icon => {
        if (theme === 'dark') {
            ...
            icon.style.filter = 'brightness(0) saturate(100%) invert(90%) sepia(9%) saturate(1500%) hue-rotate(90deg) brightness(102%) contrast(95%)';
        }
    });
    
    const searchTypeIcons = document.querySelectorAll('.search-type-icon');
    searchTypeIcons.forEach(icon => {
        if (theme === 'dark') {
            icon.style.filter = 'brightness(0) invert(1)';
        } else {
...
// ========== ДАННЫЕ ЧЕЛЛЕНДЖ ДЕМОНОВ ==========
let challengeDemons = [];
let filteredChallengeDemons = [];
let imageCache = new Map();
let allTags = new Set();
let activeTags = new Set();
let tagStats = {};

function calculateChallengePointsByRank(rank) {
...
function initializeSearch() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        const results = challengeDemons.filter(demon => 
...
function displaySearchResults(results) {
    const searchResults = document.getElementById('searchResults');
    const currentLang = localStorage.getItem('language') || 'ru';
    const noResultsText = currentLang === 'ru' ? 'Челлендж демоны не найдены' : 'Challenge demons not found';
    ...
    results.slice(0, 6).forEach(demon => {
        ...
        resultItem.innerHTML = `
            ...
            ${demon.tags && demon.tags.length > 0 ? 
                `<div style="font-size:0.8rem;color:var(--accent-challenge);margin-top:0.2rem;">${demon.tags.slice(0, 3).join(', ')}</div>` : ''
            }
        `;
...
async function getAvailableImage(demonId) {
    ...
    for (const format of imageFormats) {
        const imageUrl = `images/challenge/${demonId}.${format}`;
        
        try {
            const response = await fetch(imageUrl, { method: 'HEAD' });
            if (response.ok) {
                imageCache.set(demonId, imageUrl);
                return imageUrl;
            }
        } catch (error) {
            continue;
...
async function loadDataFromJSON() {
    try {
        console.log('📥 Загружаем челлендж демонов из challenge.json...');
        
        const response = await fetch('data/challenge.json');
        if (!response.ok) throw new Error('Файл challenge.json не найден');
        
        const data = await response.json();
        
        if (data.challengeDemons && Array.isArray(data.challengeDemons)) {
            challengeDemons = [];
            allTags.clear();
            tagStats = {};
            
            challengeDemons = data.challengeDemons.map((demon, index) => {
                const demonObj = {
                    id: demon.id || `demon${index + 1}`,
                    name: demon.name || `Unnamed Challenge ${index + 1}`,
                    creator: demon.creator || "Unknown",
            ...
            filteredChallengeDemons = [...challengeDemons];
            
            console.log('✅ Челлендж демоны загружены:', challengeDemons.length);
            console.log('🏷️  Найдено тегов:', allTags.size);
            
            const currentLang = localStorage.getItem('language') || 'ru';
            const statusText = currentLang === 'ru' ? 'Список челлендж демонов загружен' : 'Challenge demon list loaded';
            updateSyncStatus(statusText);
            
            renderChallengeDemons();
            renderTagsList();
            updateFilterStats();
            loadImagesAsync();
            ...
            throw new Error('Неверный формат challenge.json');
        }
        
    } catch (error) {
        console.error('❌ Ошибка загрузки челлендж демонов:', error);
...
async function loadImagesAsync() {
    if (challengeDemons.length === 0) return;
    
    console.log('🖼️ Асинхронная загрузка изображений челлендж демонов...');
    
    const batchSize = 5;
    
    for (let i = 0; i < challengeDemons.length; i += batchSize) {
        const batch = challengeDemons.slice(i, i + batchSize);
        
        await Promise.all(
            batch.map(async (demon) => {
                if (demon.image && demon.image.trim() !== '') {
                    try {
                        const response = await fetch(demon.image, { method: 'HEAD' });
                        if (!response.ok) {
                            demon.image = '';
                        }
                    } catch (error) {
        ...
        if (i + batchSize < challengeDemons.length) {
...
function updateFilteredDemons() {
    const searchInput = document.getElementById('searchInput');
    let filtered = challengeDemons;
    
    if (currentSearchType === 'normal') {
        if (searchInput) {
            const searchTerm = searchInput.value.toLowerCase().trim();
            if (searchTerm.length > 0) {
                filtered = challengeDemons.filter(demon => 
                    demon.name.toLowerCase().includes(searchTerm) || 
                    demon.creator.toLowerCase().includes(searchTerm)
                );
            }
    ...
    filteredChallengeDemons = filtered;
    renderChallengeDemons();
    updateFilterStats();
}
...
function updateFilterStats() {
    ...
    if (filterCount && showingCount && totalCount) {
        ...
        showingCount.textContent = filteredChallengeDemons.length;
        totalCount.textContent = challengeDemons.length;
    }
}

function renderChallengeDemons() {
    ...
    if (filteredChallengeDemons.length === 0) {
        let noResultsTitle = currentLang === 'ru' ? 'Челлендж демоны не найдены' : 'Challenge demons not found';
        let noResultsDesc = '';
            ...
            <div style="text-align: center; padding: 3rem; color: var(--text-secondary);">
                ...
                ${(currentSearchType === 'tags' && activeTags.size > 0) ? 
                    `<div style="margin-top: 1rem;">
                        <button onclick="clearAllTags()" style="background: var(--accent-challenge); color: white; border: none; padding: 0.5rem 1rem; border-radius: 6px; cursor: pointer;">
    ...
    filteredChallengeDemons.forEach(demon => {
        const points = calculateChallengePointsByRank(demon.rank);
        ...
        const hasImage = demon.image && demon.image.trim() !== '';
        
        let tagsHtml = '';
        if (demon.tags && demon.tags.length > 0) {
            tagsHtml = `<div class="demon-tags">${demon.tags.slice(0, 3).map(tag => 
                `<span class="demon-tag" onclick="event.stopPropagation(); toggleTagFilter('${tag.replace(/'/g, "\\'")}')">${tag}</span>`
            ).join('')}</div>`;
        }
        ...
        demonRow.innerHTML = `
            ...
            <div class="demon-image">
                ${hasImage ? 
                    `<img src="${demon.image}" alt="${demon.name}" 
                          loading="lazy"
                          onerror="this.onerror=null; this.parentNode.innerHTML='<div class=\'image-placeholder\'>${demon.name.charAt(0)}</div>';">` :
            ...
            <div class="demon-info">
                ...
                <div class="demon-points">${points} pts</div>
            </div>
        `;
...
function openDemonPage(demonId) {
    window.location.href = `challengedemon.html?id=${demonId}`;
}

async function refreshData() {
    console.log('🔄 Обновление данных челлендж демонов...');
    imageCache.clear();
    allTags.clear();
    activeTags.clear();
    tagStats = {};
    challengeDemons = [];
    filteredChallengeDemons = [];
...
async function initializeData() {
    ...
    document.querySelectorAll('.mobile-menu .dropdown-toggle').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            const dropdownDiv = parentLi.querySelector('.mobile-dropdown');
            if (dropdownDiv) {
                const type = dropdownDiv.id.replace('mobileDropdown', '').toLowerCase();
    ...
    const searchTypeToggle = document.getElementById('searchTypeToggle');
    if (searchTypeToggle) {
        searchTypeToggle.addEventListener('click', toggleSearchTypeMenu);
    }
...
document.addEventListener('click', function(event) {
    ...
    const searchTypeMenu = document.getElementById('searchTypeMenu');
    const searchTypeToggle = document.getElementById('searchTypeToggle');
    if (searchTypeMenu && searchTypeToggle) {
